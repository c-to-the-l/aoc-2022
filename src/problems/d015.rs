...
impl crate::Problem for Day {
    ...
    fn new(input: String) -> Self {
        Self {
            ...
            p2: 0,
            input,
            beacon_manhattans: Vec::new(),
        }
    }

    fn do_p1(&mut self) {
        let mut line_parse = nom::multi::separated_list1::<_, _, _, nom::error::Error<_>, _, _>(
        ...
        self.beacon_manhattans = out
            .iter()
            .map(|(_, sx, _, sy, _, bx, _, by)| ((*sx, *sy), manhattan(*sx, *sy, *bx, *by)))
            .collect();

        let mut ranges: Vec<(i64, i64)> = self
            .beacon_manhattans
            .iter()
            .filter_map(|((sx, sy), man)| {
